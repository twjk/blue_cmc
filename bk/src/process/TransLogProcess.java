...
public class TransLogProcess {
	...
	public void uploadOSS(){
		...
		if(dir.exists()){
			...
			for (File file : files) {
				try {
					upload = AliyunOssUtil.uploadFile("voicetrans", "translog/"+file.getName(), file);
					if(upload){
	...
	private String getDirPath(){
		...
		sbPath.append(FilePathUtil.getAbsolutePath("files"))
			  .append(FileUtil.FILE_SEP).append("translog")
			  .append(FileUtil.FILE_SEP)
		;
