	...
	<body>
		...
		<tr>
			...
			<td class="tdwhite" width="30%">
				&nbsp;
				<dict:text field="textLang" initvalue="${entity.cmcVtTrans.fromlang}"/>
				&nbsp;->&nbsp;
				<dict:text field="textLang" initvalue="${entity.cmcVtTrans.tolang}"/>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<s:if test="entity.cmcUCombo!=null">
			<td class="tdgray">使用翻译套餐：</td>
			<td class="tdwhite" colspan="3">&nbsp;${entity.cmcUCombo.combotitle}</td>
			</s:if>
			<s:else>
				<td class="tdgray">优惠（元）：</td>
				<td class="tdwhite">&nbsp;<fmt:formatNumber type="currency" pattern="0.00" value="${entity.couponamount}"/></td>
				<td class="tdgray">钱包支付（元）：</td>
				<td class="tdwhite">&nbsp;<fmt:formatNumber type="currency" pattern="0.00" value="${entity.walletamount}"/></td>
			</s:else>
		</tr>
		<tr>
			<td class="tdgray" colspan="2">计费时长（秒）：</td>
			<td class="tdwhite">&nbsp;${entity.cmcVtTrans.billingduration }（${entity.cmcVtTrans.billingMinute }分钟）</td>
			<td class="tdgray">开始计费时间：</td>
			<td class="tdwhite">&nbsp;<s:date name="entity.cmcVtTrans.startbillingtime" format="yyyy-MM-dd HH:mm:ss"/></td>
			<td class="tdgray">挂断时间：</td>
			<td class="tdwhite">&nbsp;<s:date name="entity.cmcVtTrans.handuptime" format="yyyy-MM-dd HH:mm:ss"/></td>
			
		</tr>
		<tr>
			<td class="tdgray" colspan="2">接通时长（秒）：</td>
			<td class="tdwhite">&nbsp;${entity.cmcVtTrans.connectduration }</td>
			<td class="tdgray">接通时间：</td>
			<td class="tdwhite">&nbsp;<s:date name="entity.cmcVtTrans.connecttime" format="yyyy-MM-dd HH:mm:ss"/></td>
			<td class="tdgray"></td>
			<td class="tdwhite"></td>
		...
		<tr>
			...
			<td class="tdwhite">&nbsp;<dict:text field="transVideoScene" initvalue="${entity.cmcVtTrans.scene }"/></td>
			<td class="tdgray">地址：</td>
			<td class="tdwhite" colspan="3">&nbsp;${entity.address }</td>
		</tr>		
		</table>
