function dataQuery(page) {
	...
	var url = "transPicHumanAction!doQuery.do";
...
function editData(id) {
	/*
	var url = "transPicHumanAction!doEdit.do";
	var pars = "picId=" + id;
	doEdit(url, pars);
	*/
	var url = "transPicHumanAction!doEdit.do?picId="+id;
...
function saveCancel(){
	...
	var url = "transPicHumanAction!doCancel.do";
...
//开始翻译
function startTrans(){
	var url = "transPicHumanAction!doStart.do";
...
function saveData() {
	...
	var url = "transPicHumanAction!doSave.do";
	doSave(url,"editData('"+$("#picId").val()+"')");
}
...
function finish(){
	//获取标注信息	
	$("#transResult").val(getTransInfo());
	var url = "transPicHumanAction!doFinish.do";	
...
//保存留言
function saveMsg(){
	var url = "transPicHumanAction!doAddMsg.do";
	var picId = $("#picId").val();
