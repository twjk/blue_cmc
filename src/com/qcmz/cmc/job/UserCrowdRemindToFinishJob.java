...
@Component
public class UserCrowdRemindToFinishJob extends AbstractJob {
	@Autowired
	...
	protected void work() {
		Long lockId = null;
		try {
			if(!JobConfig.USERCROWDTASK_REMINDTOFINISH_ISWORK){
				return;
			}
			...
			userCrowdTaskProcess.remindToFinish();
		} catch (Exception e) {
			logger.error("用户众包任务未完成提醒任务出错", e);
		} finally{
			if(lockId!=null){
				lockProcess.updateExpireTime(lockId, JobConfig.USERCROWDTASK_REMINDTOFINISH_VALIDTIME);
			}
		}
	}
