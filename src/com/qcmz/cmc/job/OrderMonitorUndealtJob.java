 ...
 */
@Component
public class OrderMonitorUndealtJob extends AbstractJob {
	@Autowired
	...
	protected void work() {
		Long lockId = null;
		try {
			if(!JobConfig.ORDERMONITOR_UNDEALT_ISWORK){
				return;
			}
			...
			monitorProcess.monitorUndealt();
			
			//众包任务借用一下
//			monitorProcess.monitorUserCrowdTaskVerifyOrQcUndealt();
		} catch (Exception e) {
			logger.error("未完成订单监控任务出错", e);
		} finally{
			if(lockId!=null){
				lockProcess.updateExpireTime(lockId, JobConfig.ORDERMONITOR_UNDEALT_VALIDTIME);
			}
		}
	}
