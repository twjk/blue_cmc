 ...
 */
@Component
public class OrderRefundJob extends AbstractJob {
	@Autowired
	...
	protected void work() {
		Long lockId = null;
		try {
			if(!JobConfig.ORDER_REFUND_ISWORK){
				return;
			}
			...
			logger.error("订单退款任务出错", e);
		} finally{
			if(lockId!=null){
				lockProcess.updateExpireTime(lockId, JobConfig.ORDER_REFUND_VALIDTIME);
			}
		}
	}
