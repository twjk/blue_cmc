 ...
 */
@Component
public class CrowdTaskCancelJob extends AbstractJob {
	@Autowired
	...
	protected void work() {
		Long lockId = null;
		try {
			if(!JobConfig.CROWDTASK_CANCEL_ISWORK){
				return;
			}
			...
			crowdTaskProcess.cancelTask();
		} catch (Exception e) {
			logger.error("众包任务过期取消任务出错", e);
		} finally{
			if(lockId!=null){
				lockProcess.updateExpireTime(lockId, JobConfig.CROWDTASK_CANCEL_VALIDTIME);
			}
		}
	}
