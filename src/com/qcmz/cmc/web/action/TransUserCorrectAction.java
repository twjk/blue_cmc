 ...
 * 修改历史：
 */
public class TransUserCorrectAction extends BaseAction {
	@Autowired
	...
	public String doQuery(){
		...
		transUserCorrectService.queryByMapTerm(getParameterMap(), pageBean);
	...
	public String doCorrectDst(){
		...
		try {
			transCorrectProcess.dealUserCorrect(id, dst, toLib, oper);
		} catch (Exception e) {
	...
	public String doDelete(){
		...
		try {
			transUserCorrectService.deleteUserCorrect(id, oper);
		} catch (Exception e) {
			logger.error("保存信息失败",e);
			handleResult = false;
			setResult("保存信息失败");
