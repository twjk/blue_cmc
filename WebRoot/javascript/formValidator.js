...
$.formValidator = 
{
	...
	//add by huofei 2008-12-5 desc: 用来清空已经存在的验证配置数组
	...
	initConfig : function(controlOptions)
	{
		...
		if(settings.formid!=""){
            $("#"+settings.formid).submit(function(){//绑定submit处理函数
                return $.formValidator.pageIsValid("1");
            })
        };
        
        if (jQuery_formValidator_initConfig == null ){//如果initConfig为空，创建它
            jQuery_formValidator_initConfig = new Array();
        }

        jQuery_formValidator_initConfig.push( settings );//加入setting
	},
	...
	appendValid : function(id, setting )
	{
		...
		var len = srcjo.settings.push( setting );//加新的setting
		srcjo.settings[len - 1].index = len - 1;
		return len - 1;
	},
	
	//返回相应group的initConfig
	...
	showMessage : function(returnObj)
	{
		...
		{		
            ...
            if(intiConfig.alertmessage)//如果要弹出alert框
...
$.fn.formValidator = function( msgOptions) 
{
	...
	return this.each(function()
	{
		...
		//自动形成TIP
		var tip = "";
		if(initConfig.autotip)//如果自动创建提示
		{
			if (!setting_temp.tipid){//如果没有设自动提示框的id，则将该id设为当前控件的id加Tip
                setting_temp.tipid = this.id+"Tip"
            };
			tip = setting_temp.tipid;

            if (!setting_temp.relativeid){//如果没有设置该id，则将该id设为当前控件的id
            ...
            if($("#"+tip).length==0)//如果没有创建该tip控件，则创建它。并设置位置
			{
				var formValidateTip = $("<div class='formValidateTip'></div>");
				
				if(initConfig.alertmessage == true){
		...
		if (srcTag == "INPUT" || srcTag=="TEXTAREA")
		{
			...
			jqobj.bind(setting.triggerevent, function(){   
				...
				if(returnObj.ajax >= 0)//应该是进行ajax校验
				{
...
$.fn.inputValidator = function(controlOptions)
{
	var settings = 
	{
		...
		empty:{
            leftempty:true,rightempty:true,leftemptyerror:null,rightemptyerror:null
        }
	};
