	...
	_create: function() {
		...
		if(self.options.initialize){
			...
			$.each(self.options.pictures, function( index, value ) {
				var picture = $( '#'+value );
				$(picture).append('<div style="float: left"><img src="'+$(picture).attr('picurl')+'" style="width:100%;height:100%;"></div>');
				var info = (self.options.moreInfos[value]);
				if(info!=undefined){
					// each more infos on that picture
					$.each(info, function( index, value ){
						var div = $('<div id="'+value.id+'" class="more more32"></div>' )
							.css('top',value.top*zoom).css('left',value.left*zoom).appendTo(picture);
						var imgButton;
						if(self.options.modify){
						  imgButton = $('<div class="imgButtonDrag '+self.options.button+'" title="在图片中拖拽"></div>').appendTo(div);
						  var divInput = $('<div class="inputDiv"></div>').insertAfter(imgButton);
						  var input = $('<textarea title="填写标注内容">'+value.descr+'</textarea>').appendTo(divInput);
						  $('<p class="pDelete" title="删除该标注"></p>').insertAfter(input).bind('click', function() {
						    $(div).remove();
						  });
						}else{
							imgButton = $('<div class="imgButton '+self.options.button+'"></div>').appendTo(div);
						  	var divInput = $('<div class="inputDiv"></div>').insertAfter(imgButton);
						  	var input = $('<textarea readonly="readonly">'+value.descr+'</textarea>').appendTo(divInput);
						  /*
						  	imgButton = $('<div class="imgButton '+self.options.button+'"></div>').appendTo(div);
						...
						if(self.options.modify){
							var descr;
							$('#'+value.id).draggable({
							  //containment: picture	//图片内拖动，注掉即可图片外拖动
							});
						}
					});
				}
...
//animation for tooltips
	animation: function() {
		...
		case "ltr-slide":
			...
			$(".more").hover(function(){  
				$(this).stop().animate({width: '440px'}, 200).css({'z-index' : '10'});  
			}, function () {  
		...
		default:
			//默认展示标签内容
			$(".more").addClass('ltr-slide '+self.options.animationBg).css({'z-index':'10', 'width':'440px'});
			break;
		}
	},
	...
	initialization: function(){
		...
		$.each(self.options.pictures, function( index, value ) {
			...
			var listContainer = $('<div class="listContainer" style="position:absolute;"><ul><li class="'+value+'-button '+self.options.button+'" title="在图片中拖拽"></li></ul><div class="bgList" title="choose another background for tooltips"></div><div class="buttonsList" title="choose another color for tooltips"></div></div>').appendTo(picture);
		  ...
		  $('#'+value +' .bgList').bind('click', function(){
			...
			$('#'+value).droppable({
				...
				drop: function( event, ui ) {
					plus++;
					var div = $('<div id="'+value+'-more'+plus+'" class="more more32"></div>' )
						.css('top',ui.position.top+"px").css('left',ui.position.left+"px").draggable(
						//{containment: picture}//图片内拖动
						).appendTo(picture);
					//add selected animationBg if animation set true
		      if(self.options.animation){
		        $(".more").addClass(self.options.animationBg);
		      ...
		      //workaround for firefox issue on trimming border-radius content
		      $(".more32").css('overflow','visible');
					var imgButton = $('<div class="imgButtonDrag '+self.options.button+'" title="在图片中拖拽"></div>').appendTo(div);
					var divInput = $('<div clas="inputDiv"></div>').insertAfter(imgButton);
					var input = $('<textarea title="填写标注内容"/>').appendTo(divInput).focus();
					$('<p class="pDelete" title="删除该标注"></p>').insertAfter(input).bind('click', function() {
					  $(div).remove();
					});
				}
